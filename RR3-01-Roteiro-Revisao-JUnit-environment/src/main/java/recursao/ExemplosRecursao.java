...
public class ExemplosRecursao {
	public static void main(String[] args) {
		int[] vector = {2, 4, 7, 3, 1, 8, 9};
		System.out.println(minimumRecursive(vector));
	}
	...
	public static int calcularZerosComRecursao(int[] array){
		return calcularZerosRecursivo(array, 0);
	}
	private static int calcularZerosRecursivo(int[] array, int indice){
		int zeros = 0;
	...
	private static int minimumRecursive(int[] array, int posicao) {
		...
		if(posicao == array.length-1){ //caso base 
			...
			minimum = minimumRecursive(array, posicao + 1);
			if(array[posicao] < minimum){
				minimum = array[posicao];
			}
